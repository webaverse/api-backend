...
const _areAddressesColaborator = async (addresses, hash) => {
  ...
  for (const address of addresses) {
    const [_isC, _isO1, _isO2] = await Promise.all([
      (async () => {
        try {
          const isC = await contracts.mainnetsidechain.NFT.methods
      ...
      (async () => {
        try {
          let owner = await contracts.mainnetsidechain.NFT.methods
            .ownerOf(id)
            .call();
  ...
  for (const address of addresses) {
    const [_isC, _isO1, _isO2] = await Promise.all([
      (async () => {
        try {
          const isC = await contracts.mainnetsidechain.NFT.methods
            .isSingleCollaborator(id, address)
            .call();
          // console.log('got mainnetsidechain is c', {hash, address});
          return isC;
        } catch (err) {
...
const _handleUnlockRequest = async (req, res) => {
  ...
  try {
    ...
    if (method === "OPTIONS") {
      ...
      if (ok) {
        const hash = await contracts.mainnetsidechain.NFT.methods
          .getHash(id)
          .call();
...
const _handleLockRequest = async (req, res) => {
  ...
  try {
    ...
    if (method === "OPTIONS") {
      ...
      if (
        (match = req.url.match(/^\/([0-9]+)$/)) &&
        !isNaN((id = match && parseInt(match[1], 10)))
      ) {
      ...
      if (
        ...
        if (ok) {
          const hash = await contracts.mainnetsidechain.NFT.methods
            .getHash(id)
            .call();
          const isCollaborator = await _areAddressesColaborator(
            addresses,
            hash
          );
          if (isCollaborator) {
            ...
            if (
              ...
              res.statusCode = 500;
              res.end(
                "could not interpret ciphertext for decryption: " +
...
const _isSingleCollaborator = async (tokenId, address) =>
  await _areAddressesSingleColaborator([address], tokenId);

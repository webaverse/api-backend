...
const config = require("fs").existsSync("./config.json")
  ? require("../config.json")
  : null;
...
const testnetpolygonMnemonic =
  process.env.testnetpolygonMnemonic || config.testnetpolygonMnemonic;
const infuraProjectId = process.env.infuraProjectId || config.infuraProjectId;
const encryptionMnemonic =
  process.env.encryptionMnemonic || config.encryptionMnemonic;
...
const loadPromise = (async () => {
  ...
  const web3 = {
    ...
    mainnetsidechain: new Web3(
      new Web3.providers.HttpProvider(gethNodeUrl + ":8545")
    ),
    testnet: new Web3(
      new Web3.providers.HttpProvider(
    ...
    testnetsidechain: new Web3(
      new Web3.providers.HttpProvider(gethNodeUrl + ":8546")
    ),
    polygon: new Web3(
      new Web3.providers.HttpProvider(
  ...
  const abis = await fetch("https://contracts.webaverse.com/config/abi.js")
    .then((res) => res.text())
    .then((s) => JSON.parse(s.replace(/^\s*export\s*default\s*/, "")));
  const chainIds = await fetch(
    "https://contracts.webaverse.com/config/chain-id.js"
  )
    .then((res) => res.text())
    .then((s) => JSON.parse(s.replace(/^\s*export\s*default\s*/, "")));
  const contracts = await (async () => {
    ...
    [
      ...
      [
        ...
        result[chainName][contractName] = new web3[chainName].eth.Contract(
          abis[contractName],
  ...
  const wallets = {
    mainnet: hdkey
      .fromMasterSeed(bip39.mnemonicToSeedSync(mainnetMnemonic))
      .derivePath(`m/44'/60'/0'/0/0`)
      .getWallet(),
    mainnetsidechain: hdkey
      .fromMasterSeed(bip39.mnemonicToSeedSync(mainnetMnemonic))
      .derivePath(`m/44'/60'/0'/0/0`)
      .getWallet(),
    testnet: hdkey
      .fromMasterSeed(bip39.mnemonicToSeedSync(testnetMnemonic))
      .derivePath(`m/44'/60'/0'/0/0`)
      .getWallet(),
    polygon: hdkey
      .fromMasterSeed(bip39.mnemonicToSeedSync(polygonMnemonic))
      .derivePath(`m/44'/60'/0'/0/0`)
      .getWallet(),
    testnetpolygon: hdkey
      .fromMasterSeed(bip39.mnemonicToSeedSync(testnetpolygonMnemonic))
      .derivePath(`m/44'/60'/0'/0/0`)
      .getWallet(),
  };
...
const _handleSignRequest = async (req, res) => {
  ...
  const { web3, addresses, abis, chainIds, contracts, wallets } =
    await loadPromise;
    ...
    if (method === "OPTIONS") {
      ...
      if (match) {
        ...
        const destinationChainId =
          chainIds?.[destinationChainName]?.[contractName];
        ...
        if (
          ...
          try {
            ...
            if (
              txr &&
              txr.to.toLowerCase() ===
                addresses[chainName][proxyContractName].toLowerCase()
            ) {
              const { logs } = txr;
              ...
              const log =
                logs.find(
                  (log) =>
                    (contractName === "FT" &&
                      log.topics[0] ===
                        "0x2da466a7b24304f47e87fa2e1e5a81b9831ce54fec19055ce277ca2f39ba42c4") || // WebaverseERC20Proxy Deposited
                    (contractName === "LAND" &&
                      log.topics[0] ===
                        "0x2da466a7b24304f47e87fa2e1e5a81b9831ce54fec19055ce277ca2f39ba42c4") || // WebaverseERC721Proxy Deposited
                    (contractName === "NFT" &&
                      log.topics[0] ===
                        "0x2da466a7b24304f47e87fa2e1e5a81b9831ce54fec19055ce277ca2f39ba42c4") // WebaverseERC721Proxy Deposited
                ) || null;
              ...
              if (log) {
                ...
                const proxyContractAddress =
                  addresses[destinationChainName][proxyContractName];
                ...
                const to = {
                  ...
                  v:
                    "0x" +
                    web3[destinationChainName].utils.padLeft(
                      new web3[destinationChainName].utils.BN(
                        log.topics[1].slice(2),
                        16
                      ),
                      40
                    ),
                ...
                if (contractName === "FT") {
                  const amount = {
                    t: "uint256",
                    v: new web3[destinationChainName].utils.BN(
                      log.topics[2].slice(2),
                      16
                    ),
                  };
                  const timestamp = {
                  ...
                  const message = web3[destinationChainName].utils.encodePacked(
                    to,
                    amount,
                    timestamp,
                    chainId
                  ...
                  const hashedMessage =
                    web3[destinationChainName].utils.sha3(message);
                  const sgn = web3[destinationChainName].eth.accounts.sign(
                    hashedMessage,
                  ...
                  const sgn = web3[destinationChainName].eth.accounts.sign(
                    hashedMessage,
                    wallet.getPrivateKeyString()
                  ); // await web3.eth.personal.sign(hashedMessage, address);
                  const { r, s, v } = sgn;
                  /* const r = sgn.slice(0, 66);
                            const s = '0x' + sgn.slice(66, 130);
                            const v = '0x' + sgn.slice(130, 132); */

...
const main = async () => {
  try {
    ...
    list.forEach(async (package) => {
      ...
      const webBundle = await fetch(
        `https://ipfs.exokit.org/ipfs/${details.dataHash}`
      );
      await putObject(
        "ipfs.exokit.org",
        `${details.dataHash}.wbn`,
        new Buffer.from(await webBundle.arrayBuffer())
      );
      details.icons.forEach(async (icon) => {
        let ext = mime.getExtension(icon.type);
        if (
          !ext &&
          (icon.type === "model/gltf-binary+preview" ||
            icon.type === "model/gltf-binary")
        ) {
          ext = "glb";
        }
        if (ext) {
          const preview = await fetch(
            `https://ipfs.exokit.org/ipfs/${icon.hash}`
          );
          await putObject(
            "ipfs.exokit.org",
            `${icon.hash}.${ext}`,
            new Buffer.from(await preview.arrayBuffer())
          );
        } else {

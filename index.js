    ...
    try {
      ...
      if (method === 'GET') {
        ...
        if (match = p.match(/^\/([0-9]+)$/)) {
          ...
          if (sidechainTokenIsAlsoOnMain) {
            const nftMainnetBalance = await contracts[mainnetSignature][contractName].methods.balanceOf(mainnetOrMaticAddress).call();
            const mainnetPromises = Array(nftMainnetBalance);
            for (let i = 0; i < nftMainnetBalance; i++) {
              let id = await _getChainOwnerNft(contractName)(chainName)(address, i, storeEntries);
              mainnetPromises[i] = _getChainNft(contractName)(chainName)(id.id, storeEntries);
            }
